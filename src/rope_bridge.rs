...
impl DailyProblem for RopeBridge {
    fn name(&self) -> &str { "Day 9: Rope Bridge" }
    fn index(&self) -> u8 { 9 }
	...
	(rope.tail_visited.len().to_string(), long_rope.tail_visited.len().to_string())
    }
}
...
impl Rope {
	...
	for knot_index in 0..self.knots.len() - 1 {
	    let directions = self.knots.get(knot_index).unwrap().follow_dir(self.knots.get(knot_index + 1).unwrap());
...
impl <'a> MoveIterator<'a> {
	...
	Self{ lines, current_move, index_in_move: 0 }
    }
}
...
#[cfg(test)]
mod tests {
    use super::{MoveIterator, Direction, Position, Rope};

    #[test]
    fn move_iterator() {
	let mut mi = MoveIterator::new("R 4
U 4
L 3
D 1
R 4
