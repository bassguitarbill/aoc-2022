...
impl DailyProblem for CathodeRayTube {
    fn name(&self) -> &str { "Day 10: Cathode-Ray Tube" }
    fn index(&self) -> u8 { 10 }
    fn solutions(&self, input: &str) -> (String, String) {
	let mut crt = CRT::new(input);
	(0..240).for_each(|_| crt.next_cycle());
	(
	    signal_strengths(&mut CPU::new(input), vec![20, 60, 100, 140, 180, 220]).iter().sum::<i32>().to_string(),
	    crt.pixel_string(),
	)
    }
}
...
impl <'a> CPU <'a> {
    fn new(input: &'a str) -> Self {
	Self {
	    instructions: InstructionIterator { lines: input.lines() },
	    x: 1,
	    cycle: 0,
	    adding: false,
	    to_add: 0,
...
#[cfg(test)]
mod tests {
    use super::{CPU, signal_strengths, CRT};
...
noop";
	let mut cpu = CPU::new(input);
	assert_eq!(signal_strengths(&mut cpu, vec![20, 60, 100, 140, 180, 220]), vec![420, 1140, 1800, 2940, 2880, 3960]);
    }
