...
fn common_chars(s1: &str, s2: &str) -> String {
    let mut chars = vec![];
    for c in s1.chars() {
	if s2.contains(&(c.to_string())) { chars.push(c); }
    }
    chars.iter().collect()
}
...
impl ElfGroup {
	...
	match self.badge_letter() {
	    ...
	    None => Err(format!("Common letter not found in {}, {}, and {}", self.0, self.1, self.2)),
	}
    }
}
...
fn letter_priority(input_char: char) -> Result<u32, String> {
    ...
    if input > 64 && input < 91 { Ok(input - (65 - 27)) }
    else if input > 96 && input < 123 { Ok(input - 96) }
    else { Err(format!("No priority for input char {}({})", input_char, input)) }
}
...
impl Rucksack {
	...
	match self.common_letter_between_pockets() {
	    Some(letter) => letter_priority(letter),
	    None => Err(format!("Common letter not found in {} and {}", self.0, self.1)),
	}
    }
}
...
mod tests {
	...
	assert_eq!(super::Rucksack::from_str("vJrwpWtwJgWrhcsFMMfFFhFp").unwrap().common_letter_between_pockets().unwrap(), 'p');
	assert_eq!(super::Rucksack::from_str("jqHRNqRjqzjGDLGLrsFMfFZSrLrFZsSL").unwrap().common_letter_between_pockets().unwrap(), 'L');
	assert_eq!(super::Rucksack::from_str("PmmdzqPrVvPwwTWBwg").unwrap().common_letter_between_pockets().unwrap(), 'P');
	assert_eq!(super::Rucksack::from_str("wMqvLMZHhHMvwLHjbvcjnnSBnvTQFn").unwrap().common_letter_between_pockets().unwrap(), 'v');
	assert_eq!(super::Rucksack::from_str("ttgJtRGJQctTZtZT").unwrap().common_letter_between_pockets().unwrap(), 't');
	assert_eq!(super::Rucksack::from_str("CrZsJsPPZsGzwwsLwLmpwMDw").unwrap().common_letter_between_pockets().unwrap(), 's');
    }
    ...
    #[test]
    fn badge_letter() {
	assert_eq!(super::ElfGroup("vJrwpWtwJgWrhcsFMMfFFhFp".to_string(), "jqHRNqRjqzjGDLGLrsFMfFZSrLrFZsSL".to_string(), "PmmdzqPrVvPwwTWBwg".to_string()).badge_letter().unwrap(), 'r');
	assert_eq!(super::ElfGroup("wMqvLMZHhHMvwLHjbvcjnnSBnvTQFn".to_string(), "ttgJtRGJQctTZtZT".to_string(), "CrZsJsPPZsGzwwsLwLmpwMDw".to_string()).badge_letter().unwrap(), 'Z');
    }
}
